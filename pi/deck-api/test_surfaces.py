...
d.DECK.open_episode("boot", "deck")
d.DECK.set_surface("squadrons", manual=True)
check("manual applied", sf()["active"], "squadrons")
d.DECK.set_surface("deck", manual=False)
check("auto suppressed", sf()["active"], "squadrons")
d.DECK.close_episode("boot")
check("no restore over a manual choice", sf()["active"], "squadrons")

print("\n-- the next episode starts clean --")
d.DECK.open_episode("sim", "sim")
check("auto works again", sf()["active"], "sim")
d.DECK.close_episode("sim")
check("restores to the manual choice", sf()["active"], "squadrons")
...
check("ignored", sf()["active"], "squadrons")
